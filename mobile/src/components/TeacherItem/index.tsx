...
const TeacherItem: React.FunctionComponent<TeacherItemProps> = ({
  ...
  async function toggleFavorite() {
    ...
    if (isFavorite) {
      favorites.splice(
        favorites.findIndex(
          (favorite) => (classInfo.user_id = favorite.user_id)
        ),
        1
      );
    } else {
  ...
  function openWhatsappContact() {
    Linking.openURL(`whatsapp://send?phone=+55${classInfo.whatsapp}`).catch(
      (error) => {
  ...
  return (
    <View style={styles.container}>
      ...
      <Text style={[styles.profileText, styles.title]}>{classInfo.title}</Text>
      <Text style={[styles.profileText, styles.bio]}>{classInfo.bio}</Text>
        ...
        <View style={styles.buttonsContainer}>
          <RectButton
            style={[
              ...
              styles[
                isFavorite ? 'buttonFavoriteActive' : 'buttonFavoriteInactive'
              ],
            ]}
